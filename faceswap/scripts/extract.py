...
class Extract():  # pylint:disable=too-few-public-methods
    ...
    def __init__(self, arguments: Namespace) -> None:
        ...
        # print(f"self._args.detector: {self._args.detector}\nself._args.aligner: {self._args.aligner}\nmaskers: {maskers}\n\
...
class _Extract():  # pylint:disable=too-few-public-methods
    ...
    def _run_extraction(self) -> None:
        ...
        saver = None if self._args.skip_saving_faces else ImagesSaver(self._output_dir, as_bytes=True)
        for phase in range(self._extractor.passes):
            is_final = self._extractor.final_pass
            detected_faces: dict[str, ExtractMedia] = {}
            self._extractor.launch() # Model is loaded here
            ...
            for idx, extract_media in enumerate(tqdm(self._extractor.detected_faces(), total=self._loader.process_count,
                                                     file=sys.stdout, desc=desc, leave=False)):
                self._loader.check_thread_error()
                if is_final:
