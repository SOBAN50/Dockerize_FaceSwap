...
class DetectedFace():
    ...
    def __init__(self,
        ...
        print(f"\nInitializing {self.__class__.__name__}: (image: {image.shape if image is not None and image.any() else image}, left: {left}, width: {width}, top: {top}, height: {height}, landmarks_xy: {landmarks_xy}, mask: {mask}, filename: {filename})")
        self.image = image
        self.left = left
        self.width = width
        self.top = top
