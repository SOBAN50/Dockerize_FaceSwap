...
def transform_image(image: np.ndarray, # ===========> Original Image to Output image is generated here
                    matrix: np.ndarray,
                    size: int,
    ...
    retval = cv2.warpAffine(image, mat, (size, size), flags=interpolators[0]) # => Processing here. image size: (original) and retval size: (512, 512, 3)
    return retval
...
class PoseEstimate():
    ...
    def _solve_pnp(self, landmarks: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        #print(f"\npoints: {points}, rotation: {rotation}, translation: {translation}\n")
...
class AlignedFace():
    ...
    def extract_face(self, image: np.ndarray | None) -> np.ndarray | None:
        ...
        if self._is_aligned and image.shape[0] != self._size:  # Resize the given aligned face
            ...
            retval = transform_image(image, self.matrix, self._size, self.padding) # processing done here
