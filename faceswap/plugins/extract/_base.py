...
class Extractor():
    ...
    def _thread_process(self,
        ...
        while True:
            batch = self._obtain_batch_item(function, in_queue, out_queue) # =============> batch is initialized here
            if batch is None:
                break
            ...
            if function.__name__ == "_process_output":
                ...
                for item in self.finalize(batch): # =========================> image is input from here
                    out_queue.put(item)
            else:
                out_queue.put(batch)
    ...
    def _get_item(self, queue: Queue) -> T.Literal["EOF"] | ExtractMedia | BatchType:
        ...
        if isinstance(item, ExtractMedia):
            ...
            #print(f"filename: '{item.filename}', image shape: {item.image_shape}, detected_faces: {item.detected_faces}, queue: {queue}, item: {item}\n")
